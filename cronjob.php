...
$stm = $conn->query("SELECT id, job_name, netlink_id, status, completed_date, model_name FROM print_job WHERE status = 'completed'");
...
foreach ($job_pp as $job) {
  ...
  elseif ($days_passed > 150) {
    ...
    echo $job['id'] . " - " . $job['job_name'] . " has been cancelled & deleted. \n";
  }
}
